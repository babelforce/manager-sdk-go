...
func Connect(_ context.Context, opts Options) (*ManagerClient, error) {
	...
	gc, err := managerapi.NewClientWithResponses(baseURL,
		...
		managerapi.WithRequestEditorFn(normalizeUUIDPath),
	)
	if err != nil {
		return nil, err
	}
	tac, err := taskautomationapi.NewClientWithResponses(baseURL,
		...
		taskautomationapi.WithRequestEditorFn(normalizeUUIDPath),
	)
	if err != nil {
		return nil, err
	}
	tsc, err := taskscheduleapi.NewClientWithResponses(baseURL,
		...
		taskscheduleapi.WithRequestEditorFn(normalizeUUIDPath),
	)
	if err != nil {
		return nil, err
	}
	uc, err := userapi.NewClientWithResponses(baseURL,
		...
		userapi.WithRequestEditorFn(normalizeUUIDPath),
	)
	if err != nil {
		return nil, err
	}
	au, err := authapi.NewClientWithResponses(baseURL,
		...
		authapi.WithRequestEditorFn(normalizeUUIDPath),
	)
	if err != nil {
		return nil, err
	}
