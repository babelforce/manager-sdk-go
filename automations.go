...
func (r *AutomationsResource) Clone(ctx context.Context, id string) (*managerapi.GlobalAutomationItemResponse, error) {
	resp, err := r.gc.CloneEventTriggerWithResponse(ctx, id)
	if err != nil {
		return nil, err
	}
	return result(resp.JSON201, resp.HTTPResponse, resp.Body)
...
// BulkUpdate updates several global automations (event triggers) in one request. Each item in the
// body must include its id.
func (r *AutomationsResource) BulkUpdate(ctx context.Context, body managerapi.BulkUpdateRequest) (*managerapi.ObjectListResponse, error) {
	resp, err := r.gc.BulkUpdateEventTriggersWithResponse(ctx, body)
	if err != nil {
		return nil, err
	}
	return result(resp.JSON200, resp.HTTPResponse, resp.Body)
...
func (r *AutomationsResource) BulkDelete(ctx context.Context, ids []string) (*managerapi.DefaultV2MessageResponse, error) {
	uuids, err := toUUIDs(ids)
	if err != nil {
		return nil, err
	}
	resp, err := r.gc.BulkDeleteEventTriggersWithResponse(ctx, managerapi.BulkIdsRequest{Ids: uuids})
	if err != nil {
		return nil, err
	}
	return result(resp.JSON200, resp.HTTPResponse, resp.Body)
...
func (r *AutomationsResource) Dispatch(ctx context.Context, eventTriggerID string, params *managerapi.DispatchEventTriggerParams, body map[string]any) (map[string]any, error) {
	resp, err := r.gc.DispatchEventTriggerWithResponse(ctx, eventTriggerID, params, body)
	if err != nil {
		return nil, err
	}
	data, err := result(resp.JSON200, resp.HTTPResponse, resp.Body)
