...
func TestAgentsListStalePaginationEchoTerminates(t *testing.T) {
	// A server that always echoes current=1/pages=2 with non-empty items must not cause an
	// infinite loop: the iterator advances a local page counter and stops after requesting
	// page 2 (the locally requested page reaches pages).
	var calls int
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		calls++
		w.Header().Set("Content-Type", "application/json")
		if calls > 5 { // escape hatch: a regression fails fast instead of hanging the test
			_, _ = w.Write([]byte(`{"items":[],"pagination":{"pages":2,"current":1,"total":2,"max":1}}`))
			return
		}
		_, _ = w.Write([]byte(agentPage(uuidA, "Alice", 1, 2)))
	}))
	...
	agents, err := mgr.Agents.ListAll(context.Background(), ListAgentsQuery{})
	if err != nil {
		t.Fatal(err)
	}
	if calls != 2 {
	...
	if len(agents) != 2 {
		t.Fatalf("expected 2 items (one per fetched page), got %d", len(agents))
	}
}
