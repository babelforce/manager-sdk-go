...
// List returns an iterator over users. The endpoint exposes no pagination parameters (no page/max
// query), so the SDK issues a single request and yields the response as served.
//
...
func (r *UsersResource) Me(ctx context.Context) (*managerapi.GenericItemResponse, error) {
	resp, err := r.gc.GetMeWithResponse(ctx)
	if err != nil {
		return nil, err
	}
...
func (r *UsersResource) GetByEmail(ctx context.Context, email string) (*managerapi.ManagedUserItemResponse, error) {
	resp, err := r.gc.GetUserByEmailWithResponse(ctx, email)
	if err != nil {
		return nil, err
	}
	return result(resp.JSON200, resp.HTTPResponse, resp.Body)
