...
func TestGrantFailureReturnsTypedAPIError(t *testing.T) {
	// A non-2xx token response must surface as a typed *APIError (branchable Status/Code via
	// errors.As), not an opaque fmt.Errorf — parity with Rust's ManagerError::Api.
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusBadRequest)
		_, _ = w.Write([]byte(`{"code":"invalid_client","message":"client authentication failed"}`))
	}))
	defer srv.Close()

	grants := map[string]func() error{
		"client_credentials": func() error {
			_, err := ClientCredentialsGrant(context.Background(), srv.Client(), srv.URL, "id", "wrong")
			return err
		},
		"password": func() error {
			_, err := PasswordGrant(context.Background(), srv.Client(), srv.URL, "user", "pass", "")
			return err
		},
		"refresh_token": func() error {
			_, err := RefreshTokenGrant(context.Background(), srv.Client(), srv.URL, "rt", "cid", "")
			return err
		},
		"authorization_code": func() error {
			_, err := AuthorizationCodeGrant(context.Background(), srv.Client(), srv.URL, "code", "https://app.example.com/cb", "cid", "verifier", "")
			return err
		},
	}
	for name, call := range grants {
		t.Run(name, func(t *testing.T) {
			err := call()
			if err == nil {
				t.Fatal("expected an error on a 400 token response")
			}
			var apiErr *APIError
			if !errors.As(err, &apiErr) {
				t.Fatalf("error %v (%T) is not errors.As-compatible with *APIError", err, err)
			}
			if apiErr.Status != http.StatusBadRequest || apiErr.Code != "invalid_client" {
				t.Fatalf("got Status=%d Code=%q, want 400 \"invalid_client\"", apiErr.Status, apiErr.Code)
			}
			if apiErr.Message != "client authentication failed" {
				t.Fatalf("got Message=%q, want the body's message", apiErr.Message)
			}
			if len(apiErr.Body) == 0 {
				t.Fatal("APIError.Body is empty; want the raw response body")
			}
		})
	}
}
