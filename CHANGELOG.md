...
- **Hyphenated-UUID path ids are normalized (behavioral change).** babelforce entity ids are
  unhyphenated 32-char hex on the wire; the Rust SDK already rewrote hyphenated UUIDs in request
  paths to that canonical form, Go passed them through raw (typically producing a 404). A request
  editor on every generated client now rewrites any hyphenated-UUID path segment, so
  `mgr.Calls.Get(ctx, "11111111-1111-1111-1111-111111111111")` and the unhyphenated form hit the
  same URL. Non-UUID path segments, query strings, and everything else pass through untouched.
- The divergence was found by the cross-SDK conformance harness, which this release extends to all
  five scenario families of its design (pagination, errors and entity-ids join retry and
  auth/token) plus a dispatch-drift gate. The harness is repo-internal — the published module
  contains none of it.

## v0.45.1

- **Model strictness relaxed to match the live API.** The vendored spec marked as required (or
  closed) several fields the API treats as optional (or open), so the generated models could reject
  valid production responses. Loosened and regenerated in lockstep with the TypeScript and Rust
  SDKs:
  - Pagination `Pages`, `Current`, and `Total` are now optional (`*int`). A new `pageCount(*int)`
    helper treats a nil `Pages` as `1`, and the auto-pagination loop in every resource file
    (`page >= pageCount(data.Pagination.Pages)`) stops cleanly instead of dereferencing nil.
  - `GenericItemResponse.Item` is now optional (`*map[string]any`), `Event.Id` and
    `Conference.Moderator` are no longer required, and `Conference.Members` may contain null slots
    (a member that has just left).
  - `EventType` gained `CAMPAIGN` and `CONFERENCE`; `IvrModule` gained `realtime`.
- No public API changes — a decode-robustness release.
