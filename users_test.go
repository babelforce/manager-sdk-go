...
func TestBearerAuthAndUsersListSingleRequest(t *testing.T) {
	// /api/v2/users exposes no pagination parameters, so List must issue exactly one request
	// and yield the response as served — even when the server claims more pages.
	var calls int
	var gotAuth string
	...
	if len(users) != 1 || string(users[0].Email) != "a@example.com" {
		t.Fatalf("expected the single served user, got %+v", users)
	}
	if calls != 1 {
...
func TestCreateUserStatusDrift(t *testing.T) {
	// The spec says 201 (generated field JSON201), but the live API may answer 200. The facade
	// must still decode the created object instead of reporting an error for a successful write.
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"item":{"id":"` + uuidA + `","email":"a@example.com","roles":[]},"success":true}`))
	}))
	defer srv.Close()

	mgr, _ := Connect(context.Background(), Options{BaseURL: srv.URL, Auth: Bearer("TEST")})
	out, err := mgr.Users.Create(context.Background(), managerapi.CreateManagedUserRequest{
		Email: "a@example.com",
		Roles: []managerapi.AccountRole{},
	})
	if err != nil {
		t.Fatalf("expected drift-tolerant success, got error: %v", err)
	}
	if string(out.Item.Email) != "a@example.com" || !out.Success {
